...
class Parse_output:
    def __init__(connection, package):
        '''All functions for parsing data out of RNAseq pipeline output files'''
    ...
    self.analysis_id = configSectionMap("settings")['self.analysis_id']
    self.analysis_description = configSectionMap("settings")['self.analysis_description']
     
    def add_multiple_rows(entity, data):
        ...
        Args:
            ...
            self.connection (obj): self.connection object
        Returns:
            list of added IDs
        '''
        ...
        for data in chunker(data,int(configSectionMap("settings")['max_rows'])):
            ...
            added_ids += self.connection.add(entity,data=data)
        return added_ids
    
    def parse_ena(ena_file, project):
        '''finished'''
        print ('Start ENA')
        ...
        with open(ena_file,'r', encoding="utf-8") as ena_meta:
            ...
            for line in ena_meta:
                ...
                for column, value in zip(header, column_data):
                    ...
                    data[column] = value
                to_add.append(data)
            added_ids = add_multiple_rows(entity=self.package+'ENA',data=to_add,self.connection=self.connection)
            for i, data in enumerate(to_add):
                self.connection.update_entity_rows(self.package+'Samples', data={'verifyBamID':added_ids[i]}, row_id = str(project)+'-'+str(data['run_accession'])+'-'+str(self.analysis_id))
                
    def parse_samples(sample_sheet_path, experiment_type):
        print ('Start Samples')
        ...
        for line in sample_sheet_file:
            ...
            samples_id = str(project)+'-'+str(sample_name)+'-'+str(self.analysis_id)
            ...
            try:
                self.connection.add(entity=self.package+'Sample_input_files', data=data)
            ...
            data = {'sample_id':samples_id,'project':project,'sample_name':sample_name,
                    'self.analysis_id':self.analysis_id,'experiment_type':experiment_type}
            ...
            try:
                added_ids.append(self.connection.add(entity=self.package+'Samples', data=data))
            except requests.exceptions.HTTPError as e:
                try:
        ...
        for sample_id in sample_input_files:
            self.connection.update_entity_rows(self.package+'Samples', data={'input_file_paths':sample_input_files[sample_id]}, row_id = sample_id)  
        ...
        for sh_file in sh_files:
            ...
            for tool in tools:
                ...
                try:
                    ...
                    tool_ids += self.connection.add(entity=self.package+'Tools',data=to_add)[0] + ','
                except requests.exceptions.HTTPError as e:
                    try:
                        if 'Duplicate value' in e.response.json()['errors'][0]['message']:
            ...
            if not sh_id:
                try:
                    sh_id = self.connection.add_file(file_path=basefile+'.sh', description='.sh script that was used to get the data', entity=self.package+'File',
                                data={'sample_file_id':str(project)+'-'+str(sample_name)+'-'+str(self.analysis_id)+sh_file.replace(' ','_')})
                    if isinstance(sh_id, list):
                        sh_id = sh_id[0]
                except requests.exceptions.HTTPError as e:
                    if "Duplicate value" in e.response.json()['errors'][0]['message']:
                        print('Getting the id...')
                        sh_id = self.connection.get(self.package+'File', 
                                    query=[{'field':'sample_file_id', 'operator':'EQUALS', 
                                            'value':str(project)+'-'+str(sample_name)+'-'+str(self.analysis_id)+sh_file.replace(' ','_')}])[0]['id']
                    else:
                        raise
            if not err_id:
                try:
                    try:
                        err_id = self.connection.add_file(file_path=basefile+'.err', description='file with messages printed to stderr by program', entity=self.package+'File',
                                data={'sample_file_id':str(project)+'-'+str(sample_name)+'-'+str(self.analysis_id)+(basefile+'.err').replace(' ','_')})
                    except OSError:
                        f = io.StringIO('NO FILE FOUND')
                        err_id = self.connection.add_file(file_path=basefile+'.err', io_stream=f,  description='file with messages printed to stderr by program', entity=self.package+'File',
                                data={'sample_file_id':str(project)+'-'+str(sample_name)+'-'+str(self.analysis_id)+(basefile+'.err').replace(' ','_')})
                    ...
                    if "Duplicate value" in e.response.json()['errors'][0]['message']:
                        print('Getting the id...')
                        err_id = self.connection.get(self.package+'File', 
                                    query=[{'field':'sample_file_id', 'operator':'EQUALS', 
                                        'value':str(project)+'-'+str(sample_name)+'-'+str(self.analysis_id)+(basefile+'.err').replace(' ','_')}])[0]['id']
                    else:
                        raise
            if not out_id:
                try:
                    try:
                        out_id = self.connection.add_file(file_path=basefile+'.out', description='file with messages printed to stdout by program',entity=self.package+'File',
                                data={'sample_file_id':str(project)+'-'+str(sample_name)+'-'+str(self.analysis_id)+(basefile+'.out').replace(' ','_')})
                    except OSError:
                        f = io.StringIO('NO FILE FOUND')
                        out_id = self.connection.add_file(file_path=basefile+'.out', io_stream=f, description='file with messages printed to stdout by program',entity=self.package+'File',
                                data={'sample_file_id':str(project)+'-'+str(sample_name)+'-'+str(self.analysis_id)+(basefile+'.out').replace(' ','_')})
                    ...
                    if "Duplicate value" in e.response.json()['errors'][0]['message']:
                        print('Getting the id...')
                        out_id = self.connection.get(self.package+'File', 
                                    query=[{'field':'sample_file_id', 'operator':'EQUALS', 
                                        'value':str(project)+'-'+str(sample_name)+'-'+str(self.analysis_id)+(basefile+'.out').replace(' ','_')}])[0]['id']
                    else:
                        raise
            yield sh_text, err_text, out_text, runtime, sample_name, internalId, project, sh_id, err_id, out_id, tool_ids
            
    def parse_depth_or_self(entity, depth_or_self_file, file_type):
        ...
        with open(depth_or_self_file) as verifyBamID_depth_or_self:
            ...
            depthIDs = add_multiple_rows(entity = self.package+entity, data =  to_add, self.connection = self.connection)
        return depthIDs
  
    def parse_verifyBamID(runinfo_folder_QC):
        '''finished'''
        print ('Start verifyBAMID')
        for sh_text, err_text, out_text, runtime,sample_name, internalId, project,sh_id, err_id, out_id, tool_ids in parse_rnaseq_tools(os.path.join(runinfo_folder_QC,'VerifyBamID*.sh'),self.connection, self.package):
            ...
            try:
                with open(verifyBamID_output_path+'.log') as verifyBamID_log:
                    ...
                    skipped_marker = ','.join(add_multiple_rows(entity=self.package+'Skipped_marker',data=to_add,
                    ...
                    for individual in re.findall(pattern, verifyBamID_log_text):
                        ...
                        to_add.append(data)
                    verifyBamID_individual = ','.join(add_multiple_rows(entity = self.package+'VerifyBamID_individual', data = to_add,
                                                                        self.connection = self.connection))
                depthRG = ','.join(parse_depth_or_self('DepthRG',verifyBamID_output_path+'.depthRG','depth', self.connection,self.package))
                depthSM = ','.join(parse_depth_or_self('DepthSM',verifyBamID_output_path+'.depthSM','depth', self.connection,self.package))
                selfRG = ','.join(parse_depth_or_self('SelfRG',verifyBamID_output_path+'.selfRG','self', self.connection,self.package))
                selfSM = ','.join(parse_depth_or_self('SelfSM',verifyBamID_output_path+'.selfSM','self', self.connection,self.package))
                ...
                data = {'multiple_allele_skip':multiple_allele_skip,'no_autosomal_skip':no_autosomal_skip,'sh_script':sh_id, 'depthRG':depthRG,
                        ...
                        'self_only':self_only,'self_only_sample_ID':self_only_sample_ID, 'tools':tool_ids,'sample_id':str(project)+'-'+str(sample_name)+'-'+str(self.analysis_id)}        
                try:
                    added_id = self.connection.add(self.package+'VerifyBamID', data)[0]
                except requests.exceptions.HTTPError as e:
                    if 'Duplicate value' in e.response.json()['errors'][0]['message']:
                        added_id = self.connection.get(self.package+'VerifyBamID', query=[{'field':'internalId_sampleid', 'operator':'EQUALS', 
                ...
                verifybamid_data = self.connection.get(self.package+'VerifyBamID', [{'field':'internalId_sampleid','operator':'EQUALS','value':internalId+'_'+str(project)+'-'+str(sample_name)}])
                if len(verifybamid_data) >0 and len(verifybamid_data[0]['id']) > 0:
                    added_id = verifybamid_data[0]['id']+','+added_id
                self.connection.update_entity_rows(self.package+'Samples', data={'verifyBamID':added_id}, row_id = str(project)+'-'+str(sample_name)+'-'+str(self.analysis_id))
            except OSError as e:
                print(e)
    
    def parse_hisat(runinfo_folder_QC):
        '''finished'''
        print ('Start Hisat')
        for hisat_sh_text, hisat_err_text, hisat_out_text, runtime, sample_name, internalId, project,sh_id, err_id, out_id, tool_ids in parse_rnaseq_tools(os.path.join(runinfo_folder_QC,'HisatAlignment*.sh'),self.connection, self.package):
            try:
                # paired end output
                ...
                data = {'number_of_reads':groups.group(1),'number_of_paired_reads':groups.group(2),'number_of_paired_reads_p':groups.group(3),'align_pairs_conc_0_times':groups.group(4),'align_pairs_conc_0_times_p':groups.group(5),'align_pairs_conc_1_time':groups.group(6),'align_pairs_conc_1_time_p':groups.group(7),
                    ...
                    'sh_script':sh_id,'out_file':out_id,'err_file':err_id,'runtime':runtime, 'tools':tool_ids,'sample_id':str(project)+'-'+str(sample_name)+'-'+str(self.analysis_id)}
            except AttributeError:
                 # single end output
                groups = re.search('(\d+) reads; of these:.*?'\
                ...
                try:
                    ...
                    raise
            try:
                added_id = self.connection.add(self.package+'Hisat', data)[0]
            except requests.exceptions.HTTPError as e:
                if 'Duplicate value' in e.response.json()['errors'][0]['message']:
                    added_id = self.connection.get(self.package+'Hisat', query=[{'field':'internalId_sampleid', 'operator':'EQUALS', 
                                        'value':internalId+'_'+str(project)+'-'+str(sample_name)}])[0]['id']
            hisat_data = self.connection.get(self.package+'Hisat', [{'field':'id','operator':'EQUALS','value':str(project)+'-'+str(sample_name)+'-'+str(self.analysis_id)}])
            if len(hisat_data) >0 and len(hisat_data[0]['id']) > 0:
                added_id = hisat_data[0]['id']+','+added_id
            self.connection.update_entity_rows(self.package+'Samples', data={'hisat':added_id}, row_id = str(project)+'-'+str(sample_name)+'-'+str(self.analysis_id))
   
    def parse_variantEval(runinfo_folder_QC):
        '''finished'''
        print ('Start variantEval')
        for sh_text, err_text, text, runtime, sample_name, internalId, project,sh_id, err_id, out_id, tool_ids in parse_rnaseq_tools(os.path.join(runinfo_folder_QC,'VariantEval_*.sh'), self.connection,self.package):
            table_name = None
            ...
            with open(variant_eval_file_path) as variant_eval_file:
                ...
                variant_eval_ids[entity.lower()] += add_multiple_rows(entity=self.package+entity,data=to_add,
            ...
            data = {'indel_summary':variant_eval_ids['indel_summary'], 'variant_comp_overlap':variant_eval_ids['comp_overlap'],'indel_length_histogram':variant_eval_ids['indel_length_histogram'],
                    ...
                    'runtime':runtime,'variant_class_count':variant_eval_ids['variant_class_count'],'sh_script':sh_id, 'tools':tool_ids,'sample_id':str(project)+'-'+str(sample_name)+'-'+str(self.analysis_id)}
            for key, value in data.items():
                data[key] = value.rstrip(',')
            try:
                added_id = self.connection.add(self.package+'VariantEval',data)[0]
            except requests.exceptions.HTTPError as e:
                if 'Duplicate value' in e.response.json()['errors'][0]['message']:
                    added_id = self.connection.get(self.package+'VariantEval', query=[{'field':'internalId_sampleid', 'operator':'EQUALS', 
                                    'value':internalId+'_'+str(project)+'-'+str(sample_name)}])[0]['id'] 
            variantEval_data = self.connection.get(self.package+'hisat', [{'field':'id','operator':'EQUALS','value':str(project)+'-'+str(sample_name)+'-'+str(self.analysis_id)}])
            if len(variantEval_data) >0 and len(variantEval_data[0]['id']) > 0:
                added_id = variantEval_data[0]['id']+','+added_id
            self.connection.update_entity_rows(self.package+'Samples', data={'variantEval':added_id}, row_id = str(project)+'-'+str(sample_name)+'-'+str(self.analysis_id))
            ...
            for name in files:
                if name.endswith('.md5'):
                    ...
                    for line in open(root+'/'+name):
                        ...
                        self.connection.add(self.package+'Md5sums',data)
        if x == 0:
            print('No .md5 files found in %s or its subdirectories' % project_folder)

    def parse_variantCaller(variant_caller, runinfo_folder_QC):
        ...
        if variant_caller == 'UnifiedGenotyper':
            pretext = 'Gatk'
            for sh_text, err_text, out_text, runtime, sample_name, internalId, project,sh_id, err_id, out_id, tool_ids in parse_rnaseq_tools(os.path.join(runinfo_folder_QC,pretext+variant_caller+'*.sh'),self.connection,self.package):
                ...
                with open(vcf_file) as vcf:
                    ...
                    #            snp_per_sample_id += self.connection.add(self.package+'public_rnaseq_Snp_per_sample',data = {'sample_name':sample_name,'snp_info':snp_info})+','
                    #        snp_per_sample_id = snp_per_sample_id.rstrip(',')
                    #        vcf_meta_ids['snps'] += self.connection.add(self.package+'Snp_info',{'chrom':s_l[0].replace('X','23').replace('Y','24'),'pos':s_l[1],'id':s_l[2],'ref':s_l[3],'alt':s_l[4],'qual':s_l[5],'filter':s_l[6],'info':s_l[7],'format':s_l[8],'snp_per_sample':snp_per_sample_id})+','
                    if vcf_meta_ids['contigs']:
                        contig_ids = ','.join(add_multiple_rows(entity=self.package+'Contigs',data=vcf_meta_ids['contigs'], self.connection=self.connection))
                    else:
                        contig_ids = None
                    if vcf_meta_ids['info']:
                        info_id = add_multiple_rows(entity=self.package+'Info',data=vcf_meta_ids['info'], self.connection=self.connection)
                    else:
                        info_id = None
                    if vcf_meta_ids['formats']:
                        format_ids = ','.join(add_multiple_rows(entity=self.package+'Formats',data=vcf_meta_ids['formats'], self.connection=self.connection))
                    else:
                        format_ids = None
                    if vcf_meta_ids['filters']:
                        filter_ids = ','.join(add_multiple_rows(entity=self.package+'Filters',data=vcf_meta_ids['filters'], self.connection=self.connection))
                    else:
                        filter_ids = None
                    if vcf_meta_ids['alts']:
                        alt_ids = ','.join(add_multiple_rows(entity=self.package+'Alts',data=vcf_meta_ids['alts'], self.connection=self.connection))
                    else:
                        alt_ids = None
                    if vcf_meta_ids['gvcf_block']:
                        gvcf_blck_ids = ','.join(add_multiple_rows(entity=self.package+'Gvcf_block',data=vcf_meta_ids['gvcf_block'], self.connection=self.connection))
                    else:
                        gvcf_blck_ids = None
                    ...
                    added_id = self.connection.add(self.package+'Vcf',data)
                data = {'err_file':err_id,'out_file':out_id,'runtime':runtime,'sh_script':sh_id, 'tools':tool_ids,'sample_id':str(project)+'-'+str(sample_name)+'-'+str(self.analysis_id),'type':variant_caller} # 'vcf':added_id,
                if variant_caller != 'GenotypeGvcf':
                ...
                try:
                    added_id = self.connection.add(self.package+'VariantCaller', data)[0]
                except requests.exceptions.HTTPError as e:
                    if 'Duplicate value' in e.response.json()['errors'][0]['message']:
                        added_id = self.connection.get(self.package+'VariantCaller', query=[{'field':'internalId_sampleid', 'operator':'EQUALS', 
                                    'value':internalId+'_'+str(project)+'-'+str(sample_name)}])[0]['id'] 
                if variant_caller == 'UnifiedGenotyper':
                    unifiedGenotyper_data = self.connection.get(self.package+'VariantCaller', [{'field':'id','operator':'EQUALS','value':str(project)+'-'+str(sample_name)+'-'+str(self.analysis_id)}])
                    if len(unifiedGenotyper_data) >0 and len(unifiedGenotyper_data[0]['id']) > 0:
                        added_id = unifiedGenotyper_data[0]['id']+','+added_id
                if sample_names:
                    for sample_name in sample_names:
                        self.connection.update_entity_rows(self.package+'Samples', data={'variantCaller':added_id}, row_id = str(project)+'-'+str(sample_name)+'-'+str(self.analysis_id))
                else:
                    self.connection.update_entity_rows(self.package+'Samples', data={'variantCaller':added_id}, row_id = str(project)+'-'+str(sample_name)+'-'+str(self.analysis_id))
    ...
    def parse_fastqc(runinfo_folder_QC):
        ...
        print('Start fastqc')
        for sh_text, err_text, out_text, runtime, sample_name, internalId, project,sh_id, err_id, out_id, tool_ids in parse_rnaseq_tools(os.path.join(runinfo_folder_QC,'FastQC*.sh'),self.connection, self.package):
            ...
            with zipfile.ZipFile(fastqc_outdir+os.sep+fastqc_output_base+'.zip') as archive:
                ...
                for name in archive.namelist():
                    if 'Images' in name and '.png' in name:
                        ...
                        try:
                            graphs[description] = self.connection.add_file(name, description = description, entity=self.package+"File", io_stream = img_data,
                                                                  data={'sample_file_id':str(project)+'-'+str(sample_name)+'-'+str(self.analysis_id)+name.replace(' ','_')})
                        except requests.exceptions.HTTPError:
                            graphs[description] = self.connection.get(self.package+"File",query = [{'field':'sample_file_id', 'operator':'EQUALS', 'value':str(project)+'-'+str(sample_name)+'-'+str(self.analysis_id)+name.replace(' ','_')}])
            ...
            fastqc_ids['pbsq'] = ','.join(add_multiple_rows(self.package+'Per_base_sequence_quality',to_add,self.connection))
            to_add = []
            ...
            fastqc_ids['ptsq'] = ','.join(add_multiple_rows(self.package+'Per_tile_sequence_quality',to_add,self.connection))
            to_add = []
            for line in fastqc_groups.group(10).split('\n')[1:]:
                s_l = line.split('\t')
                to_add.append({'quality':s_l[0], 'count':s_l[1]})
            fastqc_ids['psqs'] = ','.join(add_multiple_rows(self.package+'Per_sequence_quality_scores',to_add,self.connection))
            to_add = []
            ...
            fastqc_ids['pbsc'] = ','.join(add_multiple_rows(self.package+'Per_base_sequence_content',to_add,self.connection))
            to_add = []
            for line in fastqc_groups.group(12).split('\n')[1:]:
                s_l = line.split('\t')
                to_add.append({'gc_content':s_l[0], 'count':s_l[1]})
            fastqc_ids['psgc'] = ','.join(add_multiple_rows(self.package+'Per_sequence_GC_content',to_add,self.connection))
            to_add = []
            ...
            fastqc_ids['pbnc'] = ','.join(add_multiple_rows(self.package+'Per_base_N_content',to_add,self.connection))
            to_add = []
            for line in fastqc_groups.group(14).split('\n')[1:]:
                s_l = line.split('\t')
                to_add.append({'length':s_l[0], 'count':s_l[1]})
            fastqc_ids['sld'] = ','.join(add_multiple_rows(self.package+'Sequence_length_distribution',to_add,self.connection))
            ...
            fastqc_ids['sdl'] = ','.join(add_multiple_rows(self.package+'Sequence_duplication_levels',to_add,self.connection))
            to_add = []
            for line in fastqc_groups.group(16).split('\n')[1:]:
                s_l = line.split('\t')
                to_add.append({'sequence':s_l[0],'count':s_l[1],'percentage':s_l[2],'possible_source':s_l[3]})
            fastqc_ids['os'] = ','.join(add_multiple_rows(self.package+'Overrepresented_sequences',to_add,self.connection))
            to_add = []
            ...
            fastqc_ids['ac'] = ','.join(add_multiple_rows(self.package+'Adapter_content',to_add,self.connection))
            to_add = []
            ...
            fastqc_ids['kc'] = ','.join(add_multiple_rows(self.package+'Kmer_content',to_add,self.connection))
            ...
            try:
                added_id = self.connection.add(self.package+'FastQC', data)[0]
            except requests.exceptions.HTTPError as e:
                if 'Duplicate value' in e.response.json()['errors'][0]['message']:
                    added_id = self.connection.get(self.package+'FastQC', query=[{'field':'internalId_sampleid', 'operator':'EQUALS', 
                                    'value':internalId+'_'+str(project)+'-'+str(sample_name)}])[0]['id'] 
            fastqc_data = self.connection.get(self.package+'fastqc', [{'field':'id','operator':'EQUALS','value':str(project)+'-'+str(sample_name)+'-'+str(self.analysis_id)}])
            if len(fastqc_data) >0 and len(fastqc_data[0]['id']) > 0:
                added_id = fastqc_data[0]['id']+','+added_id
            self.connection.update_entity_rows(self.package+'Samples', data={'fastqc':added_id}, row_id = str(project)+'-'+str(sample_name)+'-'+str(self.analysis_id))
    
    def parse_markDuplicates(runinfo_folder_genotypeCalling):
        print ('Start markDuplies')
        for sh_text, err_text, out_text, runtime, sample_name, internalId, project,sh_id, err_id, out_id, tool_ids in parse_rnaseq_tools(os.path.join(runinfo_folder_genotypeCalling,'MarkDuplicates*.sh'),self.connection, self.package):
            ...
            with open(mark_duplicates_log) as log_file:
                ...
                if groups:
                    ...
                    markduplicates_histogram = ','.join(add_multiple_rows(self.package+'MarkDuplicates_histogram',to_add,self.connection))
                else:
                ...
                duplication_metrics = ','.join(add_multiple_rows(self.package+'Duplication_metrics', to_add,self.connection))
            ...
            added_id = self.connection.add(self.package+'MarkDuplicates', data)[0]
            self.connection.update_entity_rows(self.package+'Samples', data={'markDuplicates':added_id}, row_id = str(project)+'-'+str(sample_name)+'-'+str(self.analysis_id))
  
    def parse_bqsr(runinfo_folder_genotypeCalling):
        '''finished'''
        print ('start BQSR')
        for sh_text, err_text, out_text, runtime, sample_name, internalId, project,sh_id, err_id, out_id, tool_ids in parse_rnaseq_tools(os.path.join(runinfo_folder_genotypeCalling,'BQSR*.sh'),self.connection, self.package):
            ...
            for before_after in ['before','after']:
                ...
                for key in to_add_dict:
                    mref_ids[key] = ','.join(add_multiple_rows(self.package+key, to_add_dict[key],self.connection))
                ...
                before_after_ids[before_after] = self.connection.add(self.package+'BQSR_'+before_after+'_grp', data)[0]
            data = {'err_file':err_id,'out_file':out_id,'runtime':runtime, 'tools':tool_ids,'sample_id':str(project)+'-'+str(sample_name)+'-'+str(self.analysis_id),
                    'sh_script':sh_id,'bQSR_before_grp':before_after_ids['before'],'bQSR_after_grp':before_after_ids['after']}
            added_id = self.connection.add(self.package+'BQSR', data)[0]
            self.connection.update_entity_rows(self.package+'Samples', data={'bQSR':added_id}, row_id = str(project)+'-'+str(sample_name)+'-'+str(self.analysis_id))
 
    def parse_addOrReplaceReadGroups(runinfo_folder_genotypeCalling):
        '''finished'''
        print('start addOrReplaceReadGroups')
        for sh_text, err_text, out_text, runtime, sample_name, internalId, project,sh_id, err_id, out_id, tool_ids in parse_rnaseq_tools(os.path.join(runinfo_folder_genotypeCalling,'AddOrReplaceReadGroups*.sh'), self.connection,self.package):
            data = {'err_file':err_id,'out_file':out_id,'runtime':runtime,'internalId_sampleid':internalId+'_'+str(project)+'-'+str(sample_name),'internalId':internalId,
                    'sh_script':sh_id, 'tools':tool_ids,'sample_id':str(project)+'-'+str(sample_name)+'-'+str(self.analysis_id)}
            try:
                added_id = self.connection.add(self.package+'AddOrReplaceReadGroups', data)[0]
            except requests.exceptions.HTTPError as e:
                if 'Duplicate value' in e.response.json()['errors'][0]['message']:
                    added_id = self.connection.get(self.package+'AddOrReplaceReadGroups', query=[{'field':'internalId_sampleid', 'operator':'EQUALS', 
                                    'value':internalId+'_'+str(project)+'-'+str(sample_name)}])[0]['id'] 
                else:
                    raise
            self.connection.update_entity_rows(self.package+'Samples', data={'addOrReplaceReadGroups':added_id}, row_id = str(project)+'-'+str(sample_name)+'-'+str(self.analysis_id))
 
    def parse_samToFilteredBam(runinfo_folder_genotypeCalling):
        '''finished'''
        print('start samToFilteredBam')
        for sh_text, err_text, out_text, runtime, sample_name, internalId, project,sh_id, err_id, out_id, tool_ids in parse_rnaseq_tools(os.path.join(runinfo_folder_genotypeCalling,'SamToFilteredBam*.sh'), self.connection,self.package):
            data = {'err_file':err_id,'out_file':out_id,'runtime':runtime,'internalId_sampleid':internalId+'_'+str(project)+'-'+str(sample_name),'internalId':internalId,
                    'sh_script':sh_id, 'tools':tool_ids,'sample_id':str(project)+'-'+str(sample_name)+'-'+str(self.analysis_id)}
            try:
                added_id = self.connection.add(self.package+'SamToFilteredBam', data)[0]
            except requests.exceptions.HTTPError as e:
                if 'Duplicate value' in e.response.json()['errors'][0]['message']:
                    added_id = self.connection.get(self.package+'SamToFilteredBam', query=[{'field':'internalId_sampleid', 'operator':'EQUALS', 
                                    'value':internalId+'_'+str(project)+'-'+str(sample_name)}])[0]['id'] 
            samToFilteredBam_data = self.connection.get(self.package+'SamToFilteredBam', [{'field':'internalId_sampleid','operator':'EQUALS','value':internalId+'_'+str(project)+'-'+str(sample_name)}])
            if len(samToFilteredBam_data) >0 and len(samToFilteredBam_data[0]['id']) > 0:
                added_id = samToFilteredBam_data[0]['id']+','+added_id
            self.connection.update_entity_rows(self.package+'Samples', data={'samToFilteredBam':added_id}, row_id = str(project)+'-'+str(sample_name)+'-'+str(self.analysis_id))
   
    def parse_sortBam(runinfo_folder_genotypeCalling):
        '''finished'''
        print ('start sortBam')
        for sh_text, err_text, out_text, runtime, sample_name, internalId, project,sh_id, err_id, out_id, tool_ids in parse_rnaseq_tools(os.path.join(runinfo_folder_genotypeCalling,'SortBam*.sh'), self.connection,self.package):
            data = {'err_file':err_id,'out_file':out_id,'runtime':runtime,'internalId_sampleid':internalId+'_'+str(project)+'-'+str(sample_name),'internalId':internalId,
                    'sh_script':sh_id, 'tools':tool_ids,'sample_id':str(project)+'-'+str(sample_name)+'-'+str(self.analysis_id)}
            try:
                added_id = self.connection.add(self.package+'SortBam', data)[0]
            except requests.exceptions.HTTPError as e:
                if 'Duplicate value' in e.response.json()['errors'][0]['message']:
                    added_id = self.connection.get(self.package+'SortBam', query=[{'field':'internalId_sampleid', 'operator':'EQUALS', 
                                        'value':internalId+'_'+str(project)+'-'+str(sample_name)}])[0]['id']
            samToFilterdBam_data = self.connection.get(self.package+'SortBam', [{'field':'internalId_sampleid','operator':'EQUALS','value':internalId+'_'+str(project)+'-'+str(sample_name)}])
            if len(samToFilterdBam_data) >0 and len(samToFilterdBam_data[0]['id']) > 0:
                added_id = samToFilterdBam_data[0]['id']+','+added_id
            self.connection.update_entity_rows(self.package+'Samples', data={'sortBam':added_id}, row_id = str(project)+'-'+str(sample_name)+'-'+str(self.analysis_id))
    def parse_indelReallignmentKnown(runinfo_folder_genotypeCalling,self.connection,self.package):
        '''finished'''
        print('start indelReallignmentKnown')
        for sh_text, err_text, out_text, runtime, sample_name, internalId, project,sh_id, err_id, out_id, tool_ids in parse_rnaseq_tools(os.path.join(runinfo_folder_genotypeCalling,'IndelReallignmentKnown*.sh'), self.connection,self.package):
            data = {'err_file':err_id,'out_file':out_id,'runtime':runtime,
                    'sh_script':sh_id, 'tools':tool_ids,'sample_id':str(project)+'-'+str(sample_name)+'-'+str(self.analysis_id)}
            added_id = self.connection.add(self.package+'IndelReallignmentKnow', data)   
            self.connection.update_entity_rows(self.package+'Samples', data={'indelReallignmentKnown':added_id}, row_id = str(project)+'-'+str(sample_name)+'-'+str(self.analysis_id))
  
    def parse_gatkSplitNTrim(runinfo_folder_genotypeCalling):
        '''finished'''
        print('start gatkSplitAndtrim')
        for sh_text, err_text, out_text, runtime, sample_name, internalId, project,sh_id, err_id, out_id, tool_ids in parse_rnaseq_tools(os.path.join(runinfo_folder_genotypeCalling,'GatkSplitAndTrim*.sh'), self.connection,self.package):
            data = {'err_file':err_id,'out_file':out_id,'runtime':runtime,
                    'sh_script':sh_id, 'tools':tool_ids,'sample_id':str(project)+'-'+str(sample_name)+'-'+str(self.analysis_id)}
            added_id = self.connection.add(self.package+'GATKSplitNTrim', data)[0]
            self.connection.update_entity_rows(self.package+'Samples', data={'gATKSplitNTrim':added_id}, row_id = str(project)+'-'+str(sample_name)+'-'+str(self.analysis_id))
  
    def parse_cmMetrics(runinfo_folder, pipeline):
        print ('start cmmMetrics')
        for sh_text, err_text, out_text, runtime, sample_name, internalId, project,sh_id, err_id, out_id, tool_ids in parse_rnaseq_tools(os.path.join(runinfo_folder,'CollectMultipleMetrics*.sh'),self.connection, self.package):
            try:
                ...
                with open(cmMetrics_base_file+'.alignment_summary_metrics') as alignment_summary_metrics:
                    ...
                    for line in metrics_class[2:]:
                        ...
                        to_add.append(data)
                    alignment_summary_metrics = ','.join(add_multiple_rows(self.package+'Alignment_summary_metrics',to_add,self.connection))
                with open(cmMetrics_base_file+'.insert_size_metrics') as insert_size_metrics:
                    ...
                    for line in metrics_class[2:]:
                        ...
                        to_add.append(data)
                    insert_size_metrics_ids = ','.join(add_multiple_rows(self.package+'Insert_size_metrics_class',to_add,self.connection))
                    ...
                    for line in histogram[2:]:
                        ...
                        insert_size_histogram_ids += self.connection.add(self.package+'Insert_size_metrics_histogram',data)+','
                with open(cmMetrics_base_file+'.quality_by_cycle_metrics') as alignment_summary_metrics:
                    ...
                    for line in metrics_class[2:]:
                        ...
                        to_add.append(data)
                    quality_per_cycle_histogram_ids = ','.join(add_multiple_rows(self.package+'Quality_by_cycle_metrics',to_add,self.connection))                         
                with open(cmMetrics_base_file+'.quality_by_cycle_metrics') as alignment_summary_metrics:
                    metrics_class = alignment_summary_metrics.read().split('## HISTOGRAM')[1].split('\n')
                    ...
                    for line in metrics_class[2:]:
                        ...
                        data = {'cycle':s_l[0],'mean_quality':s_l[1]}
                        to_add.append(data)
                    quality_distribution_histogram_ids = ','.join(add_multiple_rows(self.package+'Quality_distribution_metrics',to_add,self.connection))  
                ...
                data = {'alignment_summary_metrics':alignment_summary_metrics_ids.rstrip(','),'insert_size_metrics_class':insert_size_metrics_ids.rstrip(','),'insert_size_metrics_histogram':insert_size_histogram_ids.rstrip(','),
                        ...
                        'sh_script':sh_id, 'tools':tool_ids,'sample_id':str(project)+'-'+str(sample_name)+'-'+str(self.analysis_id)}
                try:
                    added_id = self.connection.add(self.package+'CMMetrics', data)[0]
                except requests.exceptions.HTTPError as e:
                    if 'Duplicate value' in e.response.json()['errors'][0]['message']:
                        added_id = self.connection.get(self.package+'CMMetrics', query=[{'field':'internalId_sampleid', 'operator':'EQUALS', 
                                            'value':internalId+'_'+str(project)+'-'+str(sample_name)}])[0]['id']  
                if 'QC' in runinfo_folder:
                    cMMetric_data = self.connection.get(self.package+'CMMetrics', [{'field':'id','operator':'EQUALS','value':str(project)+'-'+str(sample_name)+'-'+str(self.analysis_id)}])
                    if len(cMMetric_data) >0 and len(cMMetric_data[0]['id']) > 0:
                        added_id = cMMetric_data[0]['id']+','+added_id
                self.connection.update_entity_rows(self.package+'Samples', data={'cMMetrics':added_id}, row_id = str(project)+'-'+str(sample_name)+'-'+str(self.analysis_id))
            except OSError as e:
                print(e)
                
    def parse_rMetrics(runinfo_folder, pipeline):
        '''finished'''
        print ('start rMetrics')
        for sh_text, err_text, out_text, runtime, sample_name, internalId, project,sh_id, err_id, out_id, tool_ids in parse_rnaseq_tools(os.path.join(runinfo_folder,'CollectRnaSeqMetrics*.sh'),self.connection, self.package):
            ...
            try:
                with open(rMetrics) as rMetrics_file:
                    ...
                    for line in metrics_class[2:]:
                        ...
                        to_add.append(data)
                    metrics_ids = ','.join(add_multiple_rows(self.package+'Rnaseq_metrics_class',to_add,self.connection))
                    histogram = split_file[1].split('\n')
                    ...
                    for line in histogram[2:]:
                        ...
                        metrics_histogram_ids += self.connection.add(self.package+'Rnaseq_metrics_histogram',data)+','
                ...
                data = {'rnaseq_metrics_class':metrics_ids.rstrip(','),'rnaseq_metrics_histogram':metrics_histogram_ids.rstrip(','),'err_file':err_id,'out_file':out_id,'runtime':runtime,
                        'sh_script':sh_id, 'tools':tool_ids,'sample_id':str(project)+'-'+str(sample_name)+'-'+str(self.analysis_id)}
                added_id = self.connection.add(self.package+'CRMetrics', data)[0]
                if 'QC' in runinfo_folder:
                    cRMetric_data = self.connection.get(self.package+'CRMetrics', [{'field':'id','operator':'EQUALS','value':str(project)+'-'+str(sample_name)+'-'+str(self.analysis_id)}])
                    if len(cRMetric_data) >0 and len(cRMetric_data[0]['id']) > 0:
                        added_id = cRMetric_data[0]['id']+','+added_id
                self.connection.update_entity_rows(self.package+'Samples', data={'cRMetrics':added_id}, row_id = str(project)+'-'+str(sample_name)+'-'+str(self.analysis_id))
            except OSError as e:
                print(e)
                
    def parse_mergeBam(runinfo_folder_genotypeCalling):
        '''finished'''
        print('start mergeBam')
        for sh_text, err_text, out_text, runtime, sample_name, internalId, project,sh_id, err_id, out_id, tool_ids in parse_rnaseq_tools(os.path.join(runinfo_folder_genotypeCalling,'MergeBamFiles*.sh'),self.connection, self.package):
            data = {'err_file':err_id,'out_file':out_id,'runtime':runtime,'internalId':internalId,
                    'sh_script':sh_id, 'tools':tool_ids,'sample_id':str(project)+'-'+str(sample_name)+'-'+str(self.analysis_id)}
            added_id = self.connection.add(self.package+'MergeBamFiles', data)[0]
            rRMetric_data = self.connection.get(self.package+'rRMetric', [{'field':'id','operator':'EQUALS','value':str(project)+'-'+str(sample_name)+'-'+str(self.analysis_id)}])
            if len(rRMetric_data) >0 and len(rRMetric_data[0]['id']) > 0:
                added_id = rRMetric_data[0]['id']+','+added_id
            self.connection.update_entity_rows(self.package+'Samples', data={'rRMetric':added_id}, row_id = str(project)+'-'+str(sample_name)+'-'+str(self.analysis_id))
 
    def parse_flagstat(runinfo_folder_genotypeCalling):
        '''finished'''
        print('start Flagstat')
        for sh_text, err_text, out_text, runtime, sample_name, internalId, project,sh_id, err_id, out_id, tool_ids in parse_rnaseq_tools(os.path.join(runinfo_folder_genotypeCalling,'Flagstat*.sh'),self.connection, self.package):
            ...
            try:
                ...
                raise
            try:
                added_id = self.connection.add(self.package+'Flagstat', data)
            except requests.exceptions.HTTPError:
                print('flagst send data:')
                print(data)
                raise
            self.connection.update_entity_rows(self.package+'Samples', data={'flagstat':added_id}, row_id = str(project)+'-'+str(sample_name)+'-'+str(self.analysis_id))
            
    def parse_kallisto(runinfo_folder_quantification):
        for sh_text, err_text, out_text, runtime, sample_name, internalId, project, sh_id, err_id, out_id, tool_ids in parse_rnaseq_tools(os.path.join(runinfo_folder_quantification,'Kallisto*.sh'), self.connection, self.package):
            ...
            with open(output_folder+'abundance.tsv') as abundance_tsv:
                ...
                for line in abundance_tsv:
                    ...
                    to_add.append(data)
                abundance = ','.join(add_multiple_rows(self.package+'Abundance', to_add,self.connection))
            ...
            data = {'k_mer_length':k_mer_length,'number_of_targets':number_of_targets,'number_of_kmers':number_of_kmers,'n_of_equivalence_classes':n_of_equivalence_classes,
                    ...
                    'err_file':err_id, 'tools':tool_ids,'sample_id':str(project)+'-'+str(sample_name)+'-'+str(self.analysis_id),
                    'out_file':out_id,'runtime':runtime,'sh_script':sh_id}
            added_id = self.connection.add(self.package+'CombineBedFiles', data)   
            kallisto_data = self.connection.get(self.package+'Kallisto', [{'field':'id','operator':'EQUALS','value':str(project)+'-'+str(sample_name)+'-'+str(self.analysis_id)}])
            if len(kallisto_data) >0 and len(kallisto_data[0]['id']) > 0:
                added_id = kallisto_data[0]['id']+','+added_id
            self.connection.update_entity_rows(self.package+'Samples', data={'kallisto':added_id}, row_id = str(project)+'-'+str(sample_name)+'-'+str(self.analysis_id))
            
    def parse_combineBedFiles(runinfo_folder_QC):
        '''finished'''
        print('start combineBedFiles')
        for sh_text, err_text, out_text, runtime, sample_name, internalId, project,sh_id, err_id, out_id, tool_ids in parse_rnaseq_tools(os.path.join(runinfo_folder_QC,'CombineBedFiles*.sh'), self.connection,self.package):
            ...
            data = {'mssnip_list':'NotImplemented','n_cases':groups.group(4),'n_controls':groups.group(5),'n_females':groups.group(8),
                    ...
                    'err_file':err_id, 'tools':tool_ids,'sample_id':str(project)+'-'+str(sample_name)+'-'+str(self.analysis_id),
                    'out_file':out_id,'runtime':runtime,'sh_script':sh_id}
            added_id = self.connection.add(self.package+'CombineBedFiles', data)[0]
            combineBedFiles_data = self.connection.get(self.package+'CombineBedFiles', [{'field':'id','operator':'EQUALS','value':str(project)+'-'+str(sample_name)+'-'+str(self.analysis_id)}])
            if len(combineBedFiles_data) >0 and len(combineBedFiles_data[0]['id']) > 0:
                added_id = combineBedFiles_data[0]['id']+','+added_id
            self.connection.update_entity_rows(self.package+'Samples', data={'combineBedFiles':added_id}, row_id = str(project)+'-'+str(sample_name)+'-'+str(self.analysis_id))
            
    def analyseCovariates(runinfo_folder_genotypeCalling):
        '''finished'''
        print('start analyseCovariates')
        for sh_text, err_text, out_text, runtime, sample_name, internalId, project,sh_id,err_id,out_id, tool_ids in parse_rnaseq_tools(os.path.join(runinfo_folder_genotypeCalling,'AnalyseCovariates*.sh'), self.connection,self.package):
            ...
            with open(ac_intermediate) as ac_intermediate_file:
                ...
                for line in ac_intermediate_file:
                    ...
                    data = {'covariate_value':s_l[0],'covariate_name':s_l[1],'event_type':s_l[2],'observations':s_l[3],'errors':s_l[4],
                            ...
                            'tools':tool_ids,'sample_id':str(project)+'-'+str(sample_name)+'-'+str(self.analysis_id)}
                    to_add.append(data)
                ac_intermediate_ids = ','.join(add_multiple_rows(self.package+'CovariateAnalysisTable',to_add,self.connection))
                    
                ac_intermediate_ids = ac_intermediate_ids.rstrip(',')
                
            data = {'err_file':err_id,'out_file':out_id,'runtime':runtime,
                    'sh_script':sh_id,'covariateAnalysisTable':ac_intermediate_ids, 'tools':tool_ids,'sample_id':str(project)+'-'+str(sample_name)+'-'+str(self.analysis_id)}
            added_id = self.connection.add(self.package+'AnalyseCovariates', data)[0]
            self.connection.update_entity_rows(self.package+'Samples', data={'analyseCovariates':added_id}, row_id = str(project)+'-'+str(sample_name)+'-'+str(self.analysis_id))
            
    def parse_mergeGvcf(runinfo_folder_genotypeCalling):
        '''finished'''
        print ('start mergeGvcf')
        for sh_text, err_text, mout_text, runtime, sample_name, internalId, project,sh_id,err_id,out_id, tool_ids in parse_rnaseq_tools(os.path.join(runinfo_folder_genotypeCalling,'MergeGvcf*.sh'),self.connection, self.package):
            data = {'err_file':err_id,'out_file':out_id,'runtime':runtime,
                    'sh_script':sh_id, 'tools':tool_ids,'sample_id':str(project)+'-'+str(sample_name)+'-'+str(self.analysis_id)}
            added_id = self.connection.add(self.package+'MergeGvcf', data)[0]
            self.connection.update_entity_rows(self.package+'Samples', data={'mergeGvcf':added_id}, row_id = str(project)+'-'+str(sample_name)+'-'+str(self.analysis_id))
            
    def parse_genotypeHarmonizer(runinfo_folder_genotypeCallinge):
        '''finished'''
        print('start genotypeHarmonizer')
        try:
            for sh_text, err_text, out_text, runtime, sample_name, internalId, project,sh_id, err_id, out_id, tool_ids in parse_rnaseq_tools(os.path.join(runinfo_folder_genotypeCalling,'GenotypeHarmonizer*.sh'),self.connection, self.package):
                data = {'err_file':err_id,'out_file':out_id,'runtime':runtime,'internalId_sampleid':internalId+'_'+str(project)+'-'+str(sample_name),'internalId':internalId,
                        'sh_script':sh_id, 'tools':tool_ids,'sample_id':str(project)+'-'+str(sample_name)+'-'+str(self.analysis_id)}
                try:
                    added_id = self.connection.add(self.package+'GenotypeHarmonizer', data)[0]
                except requests.exceptions.HTTPError as e:
                    if 'Duplicate value' in e.response.json()['errors'][0]['message']:
                        added_id = self.connection.get(self.package+'GenotypeHarmonizer', query=[{'field':'internalId_sampleid', 'operator':'EQUALS', 
                                        'value':internalId+'_'+str(project)+'-'+str(sample_name)}])[0]['id'] 
                genotypeHarmonizer_data = self.connection.get(self.package+'GenotypeHarmonizer', [{'field':'id','operator':'EQUALS','value':str(project)+'-'+str(sample_name)+'-'+str(self.analysis_id)}])
                if len(genotypeHarmonizer_data) >0 and len(genotypeHarmonizer_data[0]['id']) > 0:
                    added_id = genotypeHarmonizer_data[0]['id']+','+added_id
                self.connection.update_entity_rows(self.package+'Samples', data={'genotypeHarmonizer':added_id}, row_id = str(project)+'-'+str(sample_name)+'-'+str(self.analysis_id))
        except ValueError as e:
            print(str(e))
            print('Ignoring, moving on...')
            
    def parse_indelRealignmentKnown(runinfo_folder_genotypeCalling):
        '''finished'''
        print('start indelRealignmentKnown')
        for sh_text, err_text, out_text, runtime, sample_name, internalId, project,sh_id, err_id, out_id, tool_ids in parse_rnaseq_tools(os.path.join(runinfo_folder_genotypeCalling,'IndelRealignmentKnown*.sh'),self.connection, self.package):
            data = {'err_file':err_id,'out_file':out_id,'runtime':runtime,
                    'sh_script':sh_id, 'tools':tool_ids,'sample_id':str(project)+'-'+str(sample_name)+'-'+str(self.analysis_id)}
            added_id = self.connection.add(self.package+'IndelRealignmentKnown', data)[0] 
            self.connection.update_entity_rows(self.package+'Samples', data={'indelRealignmentKnown':added_id}, row_id = str(project)+'-'+str(sample_name)+'-'+str(self.analysis_id))
    
    

#if __name__ == "__main__":
#    self.connection = molgenis.Connect_Molgenis('http://localhost:8080',new_pass_file=False)
...
#    self.package = configSectionMap("settings")['self.package']
#    parse_mergeGvcf(runinfo_genotypeCalling,self.connection,self.package)
#    parse_genotypeHarmonizer(runinfo_genotypeCalling,self.connection,self.package)
#    parse_indelRealignmentKnown(runinfo_genotypeCalling,self.connection,self.package)
    
    #self.connection.delete_all_entity_rows(self.package+'_Samples')
