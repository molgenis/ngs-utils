...
if [ -f "${baits}.batch-NC_000001.11(1).bed" ]
then
...
if [ $a == 0 ]
then
	echo -e 'NC_001422.1(PhiX174)\t1\t5386\tphiX174' >> ${baits}.bed
else
...
then
	...
	then
        	...
        	chrXNONPARBed="${baits}.batch-NC_000023.11(X)np.bed"
        	chrXPARBed="${baits}.batch-NC_000023.11(X)p.bed"
		...
		position=$2
        	...
        	awk '{
        	      	if ($1 == "NC_000023.11(X)"){
				if (($2 == 1) && ($3 == 156040895)){	
					print "NC_000023.11(X)\t10001\t2781479\t+\tWGS" > "'${chrXPARBed}'" 
					print "NC_000023.11(X)\t155701383\t156030895\t+\tWGS" > "'${chrXPARBed}'" 
					print "NC_000023.11(X)\t1\t10000\t+\tWGS" > "'${chrXNONPARBed}'"
					print "NC_000023.11(X)\t2699521\t154931043\t+\tWGS" >> "'${chrXNONPARBed}'"
				}	
               	        	else if (($2 >= 10001  && $3 <= 2781479 ) || ($2 >= 155701383 && $3 <= t156030895 )){
                                	print $0 >> "'${chrXPARBed}'"
                        	} else {
		...
		if [ "${chromo}" == "NC_000023.11(X)" ]
		then
			if [[ ${position} -gt 10001 && ${position} -lt 2781479 ]] || [[ $position -gt 155701383 && $position -lt t156030895 ]]
			then
				echo $LASTLINE >> "captured.batch-NC_000023.11(X)p.bed"
			else
				echo $LASTLINE >> "captured.batch-NC_000023.11(X)np.bed"
			fi 
		else
	...
	then
		...
		then
			...
			do
				bi=$(( BATCHCOUNT + i  ))
				ba="${baits}.batch-${bi}NC_000023.11(X)"
				echo "ba=$ba bi=$bi"
				if [[ ${i} -lt 10 ]]
               			then
		...
		BATCH_Y=$((BATCH_ALL + 1))
		cat ${phiXRef} > "${baits}.batch-${BATCH_Y}NC_000024.10(Y).interval_list"
		...
		sizeOfY=$(cat "${baits}.batch-${BATCH_Y}NC_000024.10(Y).bed" | wc -l)
		if [ $sizeOfY -eq 0 ]
		then
			rm "${baits}.batch-${BATCH_Y}NC_000024.10(Y).bed"
		fi
...
if [ ! -z ${BATCH_Y+x} ]
then
	if [ -f "${baits}.batch-${BATCH_Y}NC_000024.10(Y).bed" ]
	then
        	if [ ! -f ${MAP}/captured.femaleY.bed ]
        	then
        	        echo -e 'NC_000024.10(Y)\t1\t2\t+\tFake' > ${MAP}/captured.femaleY.bed
        	fi
	fi
fi
