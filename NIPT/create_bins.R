...
parser$add_argument("-i", "--input",	type = "character",		metavar = "bed file", 		required = T,	help = "Bed file with reads.")
parser$add_argument("-o", "--output",	type = "character",		metavar = "binned data", 	required = T,	help = ".tsv file with number of reads per bin.")
parser$add_argument("-p", "--pdf",		type = "character",		metavar = "visualization",	required = T,	help = ".pdf file that visualizes distribution of reads accross bins (log-scale).")
...
write.table(bin, args$output, quote = FALSE, sep ="\t", row.names = TRUE)
