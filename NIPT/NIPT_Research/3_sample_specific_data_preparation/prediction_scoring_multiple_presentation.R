...
forward.samples <-readRDS(paste(args$ds, "/", args$type, "_Forward.rds", sep=""))
reverse.samples <- readRDS(paste(args$ds, "/", args$type, "_Reverse.rds", sep=""))
...
{
  current.file.name <- sub("^([^.]*).*", "\\1", control.file.names.all[current.sample])
  grep(pattern = current.file.name, x = control.file.names.all)
  sample.bins.f <- bins.forward.all[[current.sample]]
  sample.bins.r <- bins.reverse.all[[current.sample]]
  
  sample.bins.f[which(is.na(sample.bins.f))] = 0
  sample.bins.r[which(is.na(sample.bins.r))] = 0
  
  bins.forward <- bins.forward.all[-current.sample]
  bins.reverse <- bins.reverse.all[-current.sample]
  control.file.names <- control.file.names.all[-current.sample]
  ...
  if (args$bf > 0)
  {
    ...
    chromosomal.frac.control <- chromosomal.frac.control[best.fit.samples, ]
  }
