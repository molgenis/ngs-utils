...
while getopts "i:o:t:l:h" opt; 
do
	# shellcheck disable=SC2249
	# shellcheck disable=SC2220
	case "${opt}" in h)showHelp;; i)inputFolder="${OPTARG}";; o)outputFolder="${OPTARG}";; t)inputType="${OPTARG}";; l)validationLevel="${OPTARG}";;
esac 
done
...
then
	...
	if [[ "${whichHost}" == "wingedhelix"  || "${whichHost}" == "betabarrel" || "${whichHost}" == "copperfist" ]]
	then
		...
		then
			...
			echo "copy data /groups/umcg-gd/prm06/projects/validationVcfs/ manually to ${validationFolderTmp}"
			exit 1
		fi	
	else
		echo "please run on diagnostic cluster"
	fi
