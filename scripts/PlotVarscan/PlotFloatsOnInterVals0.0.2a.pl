...
sub DoMedianNormalisationAndDNAcopy{
	...
	$shortname =~ s/.called|.40.called//g;
	warn "Skipped DNAcopy because output is already present, if not please remove: '$shortname.seg.table'\n"&& return "$shortname.seg.table" if(-e "$shortname.seg.table" && -s "$shortname.seg.table");
...
#write.table(segment.VarscanDat.CNA.object\$out, file="$shortname.seg",  sep="\\t", row.names=FALSE)
END
...
sub MakePDFrscriptWithVCF2 {
	my $dataTable = shift @_;
	my $shortname = $dataTable;
	$shortname =~ s/.40.called|.called|.table//g;
	my $variantTable = shift @_;
	my $segTable = shift @_;
	my %ChrLenData = @_;
