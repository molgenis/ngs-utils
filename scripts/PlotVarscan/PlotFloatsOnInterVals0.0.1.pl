...
MakeVarscanDataTable($ARGV[0], %ChrLenData);
my $segtable = DoMedianNormalisationAndDNAcopy($ARGV[0] . '.table', %ChrLenData);
...
smoothed.VarscanDat.CNA.object <- smooth.CNA(CNA.object)
segment.smoothed.VarscanDat.CNA.object <- segment(smoothed.CNA.object, verbose=1)
write.table(file="$shortname.seg",  sep="\\t", row.names=FALSE)
END
	open(my $out,">","$shortname.rscript");
	print $out $rscript;
	close $out;
	warn "running DNAcopy:\n".`Rscript $shortname.rscript`;
#	$shortname.seg;
	...
	my %ChrLenData = @_;
	my $header     = <$SegHandle>;
	print $SegOutHandle 'start' . "\t" .'end' . "\t" . $header;
...
#plot
plot(VarscanDat\$offset, VarscanDat\$adjusted_log_ratio, pch='.', lty=3, xlab="", ylab="log2(tumor/normal)", xaxt="n", main="$shortname" , ylim = c(-3,2))
segments(DNAcopySegDat\$start,DNAcopySegDat\$seg.mean,DNAcopySegDat\$end,DNAcopySegDat\$seg.mean, col="white")
###i'm here^^^^^^^^^^^
#
abline(a=log(x=2,base=2),b=0, col="red")
abline(a=log(x=1.5,base=2),b=0, col="orange")
