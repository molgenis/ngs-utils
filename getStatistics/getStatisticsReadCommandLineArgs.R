if (!debug) {
	...
	arglist = c("--runtimelog", "--hsmetrics", "--alignment", "--insertmetrics", "--dedupmetrics", "--concordance", "--sample", "--colnames", "--csvout", "--tableout", "--descriptionout", "--baitsetout", "--qcdedupmetricsout", "--precise", NA)
	...
	if(wrong.flags)
	{
		...
		You may add --precise if you want to prevent the rounding of statistics.\n\n"
		)
	}
	else
	{
		...
		precise				= is.element('precise', names(args))
	}	
} else {
	nsamples = 6
