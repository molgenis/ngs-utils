...
def graph_gc(fname, outname='test.png'):
    ...
    step = 1
    widths = dict.fromkeys(range(20, 100, 20) + [99], 0)
    ...
    while low >= min_hist or high <= max_hist:
        ...
        if coverage >= 0.2 and not widths[20]:
            widths[20] = (low, high)
        if coverage >= 0.4 and not widths[40]:
            widths[40] = (low, high)
        if coverage >= 0.6 and not widths[60]:
            widths[60] = (low, high)
        if coverage >= 0.8 and not widths[80]:
            widths[80] = (low, high)
        if coverage >= 0.99 and not widths[99]:
            widths[99] = (low, high)

        low -= step
        high += step

    # make sure that value not is 0, if, then fill with (0,0)
